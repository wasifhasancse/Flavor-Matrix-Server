...
class PaymentController {
     ...
     */
    static async createMembershipSession(req, res) {
        try {
            const user = req.user;
            ...
            const sessionUrl = await payment_service_1.PaymentService.createMembershipSession(user.id, successUrl, cancelUrl);
     ...
     */
    static async createRecipeSession(req, res) {
        try {
            const user = req.user;
            ...
            const sessionUrl = await payment_service_1.PaymentService.createRecipeSession(user.id, recipeId, successUrl, cancelUrl);
            res.status(200).json({ url: sessionUrl });
        }
        catch (error) {
    ...
    static async handleWebhook(req, res) {
        ...
        try {
            await payment_service_1.PaymentService.handleWebhook(rawBody, sig);
